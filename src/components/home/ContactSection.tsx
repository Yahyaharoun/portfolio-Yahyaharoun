...
const socialLinks = [
  ...
  { icon: Twitter, label: "X (Twitter)", href: "https://x.com/yhyhaoun?s=11A1F2]" },
...
function CopyButton({ value, label = "Texte" }: { value: string, label?: string }) {
  ...
  const triggerSuccess = () => {
    setCopied(true);
    // Afficher une petite notification visuelle
    const notification = document.createElement('div');
    notification.className = 'fixed bottom-4 left-1/2 -translate-x-1/2 bg-foreground text-background px-4 py-2 rounded-full text-sm font-medium z-50 animate-in fade-in slide-in-from-bottom-4 duration-300';
    notification.textContent = `${label} copié${label.includes('Adresse') ? 'e' : ''} avec succès.`;
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-2 rounded-lg text-foreground/40 hover:text-accent hover:bg-accent/10 transition-all active:scale-95"
      aria-label="Copier"
    >
      {copied ? <Check size={16} className="text-green-500" /> : <Copy size={16} className={error ? "text-red-500" : ""} />}
    </button>
  );
}
...
export function ContactSection() {
  return (
    <section id="contact" className="mx-auto max-w-5xl px-6 py-32 scroll-mt-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-muted p-6">
            <h3 className="text-sm font-semibold text-foreground/50 uppercase tracking-wider mb-4">Contact direct</h3>
          ...
          {/* Social Links */}
          <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-muted p-6">
            <h3 className="text-sm font-semibold text-foreground/50 uppercase tracking-wider mb-4">Réseaux sociaux</h3>
        ...
        {/* Right: Contact Form */}
        <div className="lg:col-span-3 rounded-2xl border border-black/10 dark:border-white/10 bg-muted p-8">
          <h3 className="text-xl font-semibold text-foreground mb-6">Envoyer un message</h3>
          <ContactForm />
        </div>
      </div>
    </section>
